...
## route.js
* 설명: 서버 측 라우팅을 위한 코드를 포함하는 파일이. API 라우트를 정의하는 데 사용된다.
...
* 예시: 여러 페이지에 공통적으로 적용되는 템플릿을 정의하여 재사용성을 높입다.
...
## getStaticProps.js
* 설명: 정적 사이트 생성(SSG)을 위해 서버 측에서 데이터를 패치하고 페이지를 렌더링하는 함수를 정의합니다.
* 예시: 빌드 시 API에서 데이터를 가져와 정적 페이지를 생성합니다.

## getServerSideProps.js
* 설명: 서버 측에서 데이터를 패치하고 페이지를 렌더링하기 전에 실행되는 함수를 정의합니다.
* 예시: 사용자의 요청에 따라 실시간 데이터를 가져와 페이지를 생성합니다.
